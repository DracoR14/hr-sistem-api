...
# Railway fix: postgresql:// → postgresql+psycopg2://
...
def init_db():
    with engine.connect() as conn:
        ...
        # Seed demo podatke za master licencu
...
def gen_licence_key(plan="basic"):
    ...
    rand2 = ''.join(secrets.choice(chars) for _ in range(4))
    rand3 = ''.join(secrets.choice(chars) for _ in range(4))
    prefix = {"basic": "EVOS-B", "pro": "EVOS-P", "enterprise": "EVOS-E", "master": "EVOS-M"}.get(plan, "EVOS-X")
    return f"{prefix}-{rand}-{rand2}-{rand3}"
...
@app.put("/licence/{lid}/toggle")
def toggle_licenca(lid: int, user=Depends(get_current_user)):
    if user['uloga'] != 'superadmin':
        raise HTTPException(403, "Nemate pristup")
    db_exec("UPDATE licence SET aktivna = NOT aktivna WHERE id=:l", {"l": lid})
    return {"ok": True}

@app.delete("/licence/{lid}")
def del_licenca(lid: int, user=Depends(get_current_user)):
    if user['uloga'] != 'superadmin':
        raise HTTPException(403, "Nemate pristup")
    db_exec("UPDATE licence SET aktivna=false WHERE id=:l", {"l": lid})
    return {"ok": True}
...
def add_korisnik(data: KorisnikIn, user=Depends(get_current_user)):
    if user['uloga'] not in ('superadmin', 'admin'):
        raise HTTPException(403, "Nemate pristup")
    lid = data.licenca_id if user['uloga'] == 'superadmin' else user['licenca_id']
    ...
    try:
        db_exec("INSERT INTO korisnici(username,password_hash,ime,uloga,licenca_id) VALUES(:u,:p,:i,:r,:l)",
                {"u": data.username, "p": pw, "i": data.ime, "r": data.uloga, "l": lid})
        return {"ok": True}
    except:
...
def add_uposlenik(data: UposlenikIn, user=Depends(get_current_user)):
    ...
    lic = db_one("SELECT max_uposlenici FROM licence WHERE id=:l", {"l": lid(user)})
    if lic and cnt >= lic['max_uposlenici']:
        raise HTTPException(400, f"Dostigli ste maksimum od {lic['max_uposlenici']} uposlenika za vasu licencu")
