...
def init_shihtarica(user=Depends(get_current_user)):
    ...
    with engine.connect() as conn:
        ...
        ADD COLUMN IF NOT EXISTS kategorija_id INTEGER REFERENCES kategorije_uposlenika(id);
        ...
        for lic_row in licence_ids:
            ...
            for kratica, naziv, boja, je_ods, racuna in [
                ...
                conn.execute(text("""
                    INSERT INTO shihtarica_oznake
                        (kratica, naziv, boja, je_odsutnost, racuna_sate, licenca_id)
                    VALUES
                        (:k, :n, :b, :j, :r, :l)
...
def get_kategorije(user=Depends(get_current_user)):
    return db_fetch(
        "SELECT * FROM kategorije_uposlenika WHERE licenca_id=:l ORDER BY naziv",
        {"l": lid(user)}
    )
...
def add_kategorija(data: dict, user=Depends(get_current_user)):
    try:
        db_exec(
            "INSERT INTO kategorije_uposlenika(naziv, licenca_id) VALUES(:n, :l)",
...
def get_oznake(user=Depends(get_current_user)):
    return db_fetch(
        "SELECT * FROM shihtarica_oznake WHERE licenca_id=:l ORDER BY kratica",
        {"l": lid(user)}
    )
...
def get_shihtarica(godina: int, mjesec: int, user=Depends(get_current_user)):
    ...
    datum_do = f"{godina}-{mjesec:02d}-{zadnji_dan:02d}"

    uposlenici = db_fetch("""
        SELECT
            u.id,
            u.ime || ' ' || u.prezime AS ime_prezime,
            s.naziv AS sluzba,
    ...
    unosi_map = {}
    for unos in unosi:
        uid = unos["uposlenik_id"]
        if uid not in unosi_map:
            unosi_map[uid] = {}
        unosi_map[uid][unos["datum"]] = unos

    radni_dani = sum(
        1 for d in range(1, zadnji_dan + 1)
...
def save_unos(data: dict, user=Depends(get_current_user)):
    db_exec("""
        ...
        ON CONFLICT(uposlenik_id, datum) DO UPDATE SET
            ...
            sati = EXCLUDED.sati,
            prekovremeni = EXCLUDED.prekovremeni,
            nocni_rad = EXCLUDED.nocni_rad,
            pripravnost = EXCLUDED.pripravnost,
            rad_na_praznik = EXCLUDED.rad_na_praznik,
            napomena = EXCLUDED.napomena,
            kreirao = EXCLUDED.kreirao
    """, {
        "ui": data.get("uposlenik_id"),
        "da": data.get("datum"),
        "oz": data.get("oznaka", "8"),
        "sa": data.get("sati", 8.0),
        "pr": data.get("prekovremeni", 0),
        "no": data.get("nocni_rad", 0),
        "pp": data.get("pripravnost", 0),
        "rp": data.get("rad_na_praznik", 0),
        "na": data.get("napomena", ""),
        "l": lid(user),
        "kr": user["id"],
    })
